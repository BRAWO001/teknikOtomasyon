...
/* ========================
   ✅ Site helpers (tüm formatlara dayanıklı)
======================== */
function asArray(v) {
  return Array.isArray(v) ? v : [];
}
function getSiteId(s) {
  return s?.id ?? s?.Id ?? s?.siteId ?? s?.SiteId ?? s?.pkNo ?? s?.PkNo ?? null;
}
function getSiteAd(s) {
  return (
    s?.ad ??
    s?.Ad ??
    s?.siteAdi ??
...
function TalepDosyaPanel({ talepId, seriNo, onStatusChange, disabled }) {
  ...
  return (
    <section className="space-y-3 rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      {/* (manual attach butonu istersen aç) */}
      {pending.length > 0 && _talepId ? (
        <div className="flex justify-end">
          <button
            type="button"
            onClick={handleAttachPending}
            disabled={disabled || uploading || attaching}
            className="rounded-md border border-amber-400 bg-amber-50 px-3 py-1.5 text-[11px] font-semibold text-amber-800 hover:bg-amber-100 disabled:opacity-60"
          >
            Bekleyen Dosyaları Talebe Bağla
          </button>
        </div>
      ) : null}
    </section>
...
export default function YeniSatinAlmaPage() {
  ...
  // ✅ İSTEK: options'da TÜM SİTELER görünsün → lock yok
  const isSingleSiteLocked = false;

  const selectedSiteName = useMemo(() => {
    const sid = siteId ? Number(siteId) : null;
    if (!sid) return null;
    const s = asArray(sites).find((x) => Number(getSiteId(x)) === sid);
    return s ? getSiteAd(s) : null;
  }, [siteId, sites]);
  ...
  // ------------------------------------------------------
  // ✅ Lookups: SİTELERİ MUTLAKA TÜMÜNÜ ÇEK ve göster
  // - önce lookups dene
  // - boşsa fallback SiteAptEvControllerSet/sites dene
  // - yine boşsa bir hata mesajı setle
  // ------------------------------------------------------
  useEffect(() => {
    let cancelled = false;
    ...
    const fetchLookups = async () => {
      ...
      try {
        ...
        try {
          lookupsRes = await getDataAsync(`Personeller/satinalma-yeni/lookups?personelId=${personelId}`);
        } catch (e) {
        ...
        let resSites = [];
        if (lookupsRes) {
          resSites = asArray(lookupsRes?.sites ?? lookupsRes?.Sites);

          if (!resSites.length) {
            const dataNode = lookupsRes?.data ?? lookupsRes?.Data;
            resSites = asArray(dataNode?.sites ?? dataNode?.Sites);
          }
        }

        // 2) Fallback: tüm siteler
        if (!resSites.length) {
          try {
            const fallbackSites = await getDataAsync("SiteAptEvControllerSet/sites");
            resSites = asArray(fallbackSites);
          } catch (e) {
            console.warn("fallback sites endpoint hata:", e);
            resSites = [];
          }
        }
        ...
        // 3) Onaycılar (senin mantık)
        let onaycilar = [];
        try {
          const onayRes = await getDataAsync(`Personeller/satinalma/onaycilar`);
          if (Array.isArray(onayRes)) onaycilar = onayRes;
          else if (Array.isArray(onayRes?.onaycilar)) onaycilar = onayRes.onaycilar;
          else if (Array.isArray(onayRes?.Onaycilar)) onaycilar = onayRes.Onaycilar;
        } catch (innerErr) {
          console.warn("Yeni onaycı endpointi hata verdi, lookups içindekiler kullanılacak.");
          onaycilar = asArray(lookupsRes?.onaycilar ?? lookupsRes?.Onaycilar);
        }

        if (cancelled) return;
        setOnayciCandidates(onaycilar);

        // 4) Default site seçimi (varsa)
        const defaultSiteId =
          (lookupsRes?.defaultSiteId ??
            lookupsRes?.DefaultSiteId ??
            (lookupsRes?.data?.defaultSiteId ?? lookupsRes?.Data?.DefaultSiteId) ??
            null) ?? null;

        // hiç seçili yoksa default ata
        if (!siteId && defaultSiteId) {
          setSiteId(String(defaultSiteId));
        }

        // eğer sites hala boşsa uyarı
        if (!resSites.length) {
          setError("Site/Proje listesi alınamadı. (sites boş döndü)");
        }
      } catch (err) {
        if (cancelled) return;
        console.error("LOOKUP FETCH ERROR:", err);
        setError("Proje/Site veya onaycı listesi alınırken bir hata oluştu.");
      } finally {
  ...
  useEffect(() => {
    ...
    const exists = (onayciCandidates || []).some((p) => (p?.id ?? p?.Id) === defaultId);
  ...
  const handleRowChange = (index, field, value) => {
    setMalzemeler((prev) => prev.map((row, i) => (i === index ? { ...row, [field]: value } : row)));
  };
  ...
  useEffect(() => {
    ...
    const canRedirect =
      !panelStatus.uploading && !panelStatus.attaching && (panelStatus.pendingCount || 0) === 0;
  ...
  const handleSubmit = async (e) => {
    ...
    if (isProcurementLike) {
      ...
      if (!malzemeIstemiyorum) {
        for (let i = 0; i < malzemeler.length; i++) {
          ...
          if (!malzemeAdi || !marka || !birim || !kullanimAmaci || !notText) {
            setError(`Malzeme satır ${i + 1} için örnek ürün linki hariç tüm alanlar zorunludur.`);
            return;
          }

          if (!adetNum || adetNum <= 0) {
            setError(`Malzeme satır ${i + 1} için adet alanı zorunludur ve 0'dan büyük olmalıdır.`);
            return;
          }
      ...
      try {
        ...
        const createdId = result?.id ?? result?.Id ?? result?.satinAlmaId ?? result?.SatinAlmaId;
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-lg bg-white p-4 shadow-sm">
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              {/* ✅ Site / Proje (tüm siteler options'da) */}
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-zinc-700">
                ...
                >
                  ...
                  {asArray(sites).map((s, i) => {
                    ...
                    const ad = getSiteAd(s);
                    const key = id ?? `${ad ?? "site"}-${i}`;
                    return (
            ...
            <div className="pt-2">
              <TalepDosyaPanel
                ...
                onStatusChange={(st) => setPanelStatus(st)}
