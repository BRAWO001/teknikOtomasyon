// pages/projeGorevlileri/projeGorevlileriDuyurular.jsx
...
export default function ProjeGorevlileriDuyurularPage() {
  ...
  // burada site seçimi kullanıcı whitelist'inden otomatik gelir
  ...
  // personelin bağlı olduğu site(ler)i al
  useEffect(() => {
    let cancelled = false;
  ...
  const endpoint = useMemo(() => {
    ...
    // site dışarıdan / whitelist mantığıyla geliyor
  ...
  const selectedSiteName =
    siteList.find((x) => String(x?.SiteId ?? x?.siteId ?? x?.id ?? x?.Id) === String(safeSiteId))
      ?.Site?.Ad ||
    siteList.find((x) => String(x?.SiteId ?? x?.siteId ?? x?.id ?? x?.Id) === String(safeSiteId))
      ?.site?.ad ||
