...
// pages/projeGorevlileri/projeGorevlileriKararlar.jsx
...
export default function ProjeGorevlileriKararlarPage() {
  ...
  useEffect(() => {
    ...
    const loadSites = async () => {
      ...
      try {
        ...
        // ✅ cookie siteId varsa ama whitelist'te değilse IGNORE
        ...
        // ✅ mevcut seçili site whitelist'te değilse düşür
        ...
        // ✅ siteId yoksa otomatik ilk siteyi seç
        if (!siteId) {
  ...
  const safeSiteId = useMemo(() => {
    ...
    // siteId bozulduysa ilk whitelist'e düş
  ...
  // ✅ kullanıcı seçimi bozulduysa state'i düzelt
  useEffect(() => {
  ...
  const endpoint = useMemo(() => {
    // ✅ siteId zorunlu! yoksa endpoint kurma
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      console.error("KARAR LIST GET ERROR:", e);
      setItems([]);
      setTotalPages(1);
      setTotalCount(0);
  ...
  const goYeniKarar = () => router.push("/projeGorevlileri/ProjeSorumlusuYeniKarar");
