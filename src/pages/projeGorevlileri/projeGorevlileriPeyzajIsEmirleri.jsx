...
export default function ProjeGorevlileriPeyzajIsEmirleriPage() {
  ...
  useEffect(() => {
    ...
    const loadPersoneller = async () => {
      try {
        const res = await getDataAsync("Personeller/ByDurum?rolKod=33&aktifMi=true");
        if (cancelled) return;
  ...
  async function loadData() {
    ...
    try {
      ...
      console.error("Proje Görevlileri Peyzaj İşleri yükleme hatası:", err);
      setItems([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-[13px] font-semibold text-zinc-800 dark:text-zinc-100">
              Proje Görevlileri – Peyzaj İşleri
            </div>
