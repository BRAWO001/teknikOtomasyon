...
const PEYZAJ_DURUM_STEPS = [
  { value: 10, label: "10 - Beklemede" },
  { value: 20, label: "20 - İşe Başlandı" },
  { value: 30, label: "30 - Devam Ediyor" },
  { value: 50, label: "50 - Malzeme Temini" },
  { value: 60, label: "60 - Kontrol Aşamasında" },
  { value: 75, label: "75 - Teslim Hazır" },
  
  { value: 100, label: "100 - İş Bitirildi" },
];

export default function PeyzajIsEmriDurumGuncelleModals({
  isOpen,
  onClose,
  isEmriId,
  isEmriKod,
  ...
  const submitDurum = async (durumToSend) => {
    try {
      ...
      await postDataAsync(`peyzaj-is-emri-formu/${isEmriId}/durum-guncelle`, {
        personelId,
        yeniDurumKod: next,
      });
      ...
      console.error("Peyzaj durumu güncellenirken hata:", err);
      setError(
            ...
            <div className="space-y-3 px-4 py-4">
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-zinc-200 bg-zinc-50 p-1 text-sm text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800/60 dark:text-zinc-100">
                ...
                <span className="font-medium">
                  Kullanılan malzeme varsa girişi yapıldı mı?
                </span>
              </label>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-zinc-200 bg-zinc-50 p-1 text-sm text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800/60 dark:text-zinc-100">
                ...
                />
                <span className="font-medium">
                  İş açıklamaları / notlar bırakıldı mı?
                </span>
              </label>
            </div>
